...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	for KEY in "${KEYS[@]}"; do
		cvmd add-genesis-account $KEY 10000000000000000000000000000aCVM --keyring-backend $KEYRING --home "$HOMEDIR"
		# cvmd add-genesis-account $KEY 100000000000000000000000000aCVM --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	cvmd gentx ${KEYS[0]} 100000000000000000000000aCVM --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
